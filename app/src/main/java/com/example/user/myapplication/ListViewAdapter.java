...
public class ListViewAdapter extends BaseAdapter {
    ...
    private ArrayList<Prom> listViewItemList = new ArrayList<Prom>() ;
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        CheckBox chk = (CheckBox)convertView.findViewById(R.id.check1);
        // Data Set(listViewItemList)에서 position에 위치한 데이터 참조 획득
        Prom listViewItem = listViewItemList.get(position);

        // 아이템 내 각 위젯에 데이터 반영
        id.setText(listViewItem.getName());
        score.setText(listViewItem.getScore()+"");
        content.setText(listViewItem.getContent());
        i=listViewItem.getSuc();
        if(i==1)
        {
            chk.setChecked(true);
        }
    ...
    public void addItem(String name, int score, String content, int suc) {
        Prom item = new Prom();
